...
for (A in 1:length(z_set)) {
  ...
  #######################################
  new_folder <- paste0(feature_selection, '_CA-SA+EUP')
  quick <-
    '~/Desktop/Github/Data/Complex Aneuploid vs Single Aneuploid + Euploid/'
  ...
  ANU_data <- imp_data[imp_data$PGD_RESULT == 'ANU', ]
  ANU_index <-
    createDataPartition(ANU_data$ab_or_norm,
                        p = 0.70,
                        list = FALSE,
                        times = 1)
  ANU_train <- ANU_data[ANU_index, ]
  ANU_val <- ANU_data[-ANU_index, ]
  ...
  CxA_train <- rbind(CxA_train, CxA_train)
  CxA_val <- CxA_data[-CxA_index, ]
  
  training <- rbind(CxA_train, EUP_train, ANU_train)
  training$ab_or_norm <-
    ifelse(training$PGD_RESULT == "CxA", "Abnormal", "Normal")
  ...
  if (decide == T) {
    ...
    training <- rbind(CxA_train, EUP_train, ANU_train, rand_dbl)
    training$ab_or_norm <-
      ifelse(training$PGD_RESULT == "CxA", "Abnormal", "Normal")
  } else if (decide == F) {
    combo_data <- rbind(EUP_train, ANU_train)
    rand_dbl <-
      combo_data[sample(nrow(combo_data), bal_num, replace = F), ]
    training <- rbind(CxA_train, EUP_train, ANU_train, rand_dbl)
  }
  
  training <- training[, -c(ncol(training))]
  
  validation <- rbind(CxA_val, EUP_val, ANU_val)
  validation$ab_or_norm <-
    ifelse(validation$PGD_RESULT == "CxA", "Abnormal", "Normal")
  ...
  ANU_VT <-
    validation_test_impute[validation_test_impute$PGD == "ANU", ]
  ANU_VT_index <-
  ...
  validation_impute <- rbind(EUP_VT_val, CxA_VT_val, ANU_VT_val)
  validation_impute <-
    validation_impute[, -c(ncol(validation_impute))]
  test_impute <- rbind(EUP_VT_test, CxA_VT_test, ANU_VT_test)
