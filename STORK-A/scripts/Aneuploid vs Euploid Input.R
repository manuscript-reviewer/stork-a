...
for (A in 1:length(z_set)) {
  ...
  #######################################
  new_folder <- paste0(feature_selection, '_ANU-EUP')
  quick <- '~/Desktop/Github/Data/Aneuploid vs Euploid/'
  ...
  #imp_data <- imp_data[-c(which(imp_data$PGD_RESULT == 'MUT')), ]
  
  #create 70/30 split by class for training a validaiton (validation will later be split into validation and test)
  ANU_data <- imp_data[imp_data$ab_or_norm == 'Abnormal', ]
  ANU_index <-
    createDataPartition(ANU_data$ab_or_norm,
                        p = 0.70,
                        list = FALSE,
                        times = 1)
  ANU_train <- ANU_data[ANU_index, ] #70 percent of data training
  ANU_val <-
    ANU_data[-ANU_index, ] #30 perent of data for val and test
  
  EUP_data <- imp_data[imp_data$ab_or_norm == 'Normal', ]
  EUP_index <-
  ...
  training <- rbind(ANU_train, EUP_train)
  ...
  if (decide == T) {
    rand_dbl <-
      ANU_train[sample(nrow(ANU_train), bal_num, replace = F), ]
  ...
  validation <- rbind(ANU_val, EUP_val)
  validation <- validation[, -c(ncol(validation))]
  
  #impute missing data using media imputation
  ...
  EUP_VT <-
    validation_test_impute[validation_test_impute$ab_or_norm == "Normal", ]
  EUP_VT_index <-
  ...
  ANU_VT <-
    validation_test_impute[validation_test_impute$ab_or_norm == "Abnormal", ]
  ANU_VT_index <-
    createDataPartition(ANU_VT$ab_or_norm,
                        p = 0.50,
                        list = FALSE,
                        times = 1)
  ANU_VT_val <- ANU_VT[ANU_VT_index, ]
  ANU_VT_test <- ANU_VT[-ANU_VT_index, ]
  
  validation_impute <- rbind(EUP_VT_val, ANU_VT_val)
  test_impute <- rbind(EUP_VT_test, ANU_VT_test)
  
  make_input <- function(x) {
    x$Label <- ifelse(x$ab_or_norm == 'Abnormal', 0, 1)
    x$img_file <-
      data$img_file[match(x$SUBJECT_NO, data$SUBJECT_NO)]
    fx <- c("img_file", 'Label')
    fy <- input_names
    x <- x[, c(fx, fy)]
